...
class PytorchPrepWordLevelWithSyllable:
    """Prepare NECTEC data for pytorch"""
    def __init__(self,train_set,dev_set,test_set):
        self.train_set = train_set
        self.dev_set = dev_set
        self.test_set = test_set
        self.word_to_index, self.syl_to_index, self.char_to_index, self.word_count = self.__word2index()
        ...
        self.input_train_word, self.input_train_syl, self.pos_train, self.ner_train = self._split_input_label(self.target_train_set)
        self.input_dev_word,self.input_dev_syl, self.pos_dev, self.ner_dev = self._split_input_label(self.target_dev_set)
        self.input_test_word,self.input_test_syl ,self.pos_test, self.ner_test = self._split_input_label(self.target_test_set)
        ...
        all_ner = sorted(list(set([val for sublist in self.ner_train+self.ner_dev+self.ner_test for val in sublist])))
        self.ner_to_index = dict((c, i) for i, c in enumerate(all_ner))

        self.index_to_pos = dict((v,k) for k,v in self.pos_to_index.items())
        self.index_to_ner = dict((v,k) for k,v in self.ner_to_index.items())
    ...
    def __word2index(self):
        ...
        ##all syllables
        all_syllables = sorted(set(syllable_list))
        all_syllables.append("UNK")
        syl_to_index = dict((c, i) for i, c in enumerate(all_syllables))
        ...
        return word_to_index, syl_to_index, char_to_index, word_count
        ...
        for text in input_set:
            ...
            for word in text:
                ...
                if len(word)>1:
                    pos_class=word[2]
                    ner_class=word[3]
                    temp.append((word[0],word[1],pos_class,ner_class))
            ...
            for word in text:
                ...
                if len(word)>1:
                    ...
                    if word[0] in self.word_to_index:
                        ...
                        oov=True
                    temp.append((word[0],word[1],pos_class,ner_class,oov))
            target_set.append(temp)#for each text chunk/file
        return target_set
    ...
    def _split_input_label(self,target_set):
        ...
        for text in target_set:
            ...
            for word in text:
                ...
                temp_syl.append(word[1])
                temp_pos.append(word[2])
                temp_ner.append(word[3])
            input_word.append(temp_input)
            input_syl.append(temp_syl)
            pos.append(temp_pos)
            ner.append(temp_ner)
        return input_word, input_syl, pos, ner
    ...
    def prepare_sequence_syl(self, input_word):
        idxs = [self.syl_to_index[s] for s in input_word]
        tensor = torch.tensor(idxs, dtype=torch.long)
        return tensor.to(device)

    def prepare_sequence_test_syl(self, test_word):
        idxs=list()
        for s in test_word:
            if s in self.syl_to_index:
                idxs.append(self.syl_to_index[s])
            else:
                idxs.append(self.syl_to_index["UNK"])
        tensor = torch.tensor(idxs, dtype=torch.long)
        return tensor.to(device)
    ...
    def prepare_sequence_target_ner(self, input_label):
        idxs = [self.ner_to_index[w] for w in input_label]
        tensor = torch.tensor(idxs, dtype=torch.long)
        return tensor.to(device)
