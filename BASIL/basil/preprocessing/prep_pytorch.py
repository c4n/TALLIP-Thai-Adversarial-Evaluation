...
class PytorchPrepWordLevel:
    ...
    def __init__(self,train_set,dev_set,test_set):
        ...
        self.input_train_word, self.pos_train, self.ner_train = self._split_input_label(self.target_train_set)
        self.input_dev_word, self.pos_dev, self.ner_dev = self._split_input_label(self.target_dev_set)
        self.input_test_word, self.pos_test, self.ner_test = self._split_input_label(self.target_test_set)
        ...
        for text in input_set:
            ...
            for word in text:
                ...
                if len(word)>1:
                    pos_class=word[1]
                    ner_class=word[2]
                    temp.append((word[0],pos_class,ner_class))
            ...
            for word in text:
                ...
                if len(word)>1:
                    ...
                    if word[0] in self.word_to_index:
                        ...
                        oov=True
                    temp.append((word[0],pos_class,ner_class,oov))
            target_set.append(temp)#for each text chunk/file
        return target_set
    ...
    def _split_input_label(self,target_set):
        ...
        for text in target_set:
            ...
            for word in text:
                ...
                temp_ner.append(word[2])
            input_word.append(temp_input)
            pos.append(temp_pos)
            ner.append(temp_ner)
        return input_word, pos, ner
