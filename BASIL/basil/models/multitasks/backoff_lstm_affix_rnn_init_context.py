...
class LSTMTagger(nn.Module):
    ...
    def __init__(self,char_embedding_dim, word_embedding_dim, hidden_dim, char_size, word_size, prefix_size, tagset_size_pos, tagset_size_ner):
        ...
        self.lstm_pos_ner = nn.LSTM(word_embedding_dim+hidden_dim, hidden_dim // 2, num_layers = 2, dropout = 0.1, bidirectional = True)
    ...
    def init_hidden_pos_ner(self):
        ...
        return (torch.zeros(4, 1, self.hidden_dim // 2,requires_grad=True).to(device),
                torch.zeros(4, 1, self.hidden_dim // 2,requires_grad=True).to(device))
