...
class LSTMTagger(nn.Module):
    ...
    def __init__(self,char_embedding_dim, word_embedding_dim, hidden_dim, char_size, word_size, prefix_size, tagset_size_pos, tagset_size_ner):
        ...
        self.share_rep_size = math.ceil(hidden_dim/3)
        self.hidden2tag_pos = nn.Linear(hidden_dim - hidden_dim//3, tagset_size_pos)
        self.hidden2tag_ner = nn.Linear(hidden_dim - hidden_dim//3, tagset_size_ner)
    ...
    def _lstm_pos_ner_feats(self,word_char_cat,prefix_emb):
        ...
        #condition with prefix
        lstm_out= torch.tanh(lstm_out+prefix_emb.view(len(word_char_cat), 1, -1))


        #decomposing
        lstm_out_pos = lstm_out.narrow(2,0,(self.hidden_dim//3)+self.share_rep_size)
        lstm_out_ner = lstm_out.narrow(2,self.hidden_dim//3,(self.hidden_dim//3)+self.share_rep_size)

        tag_space_pos = self.hidden2tag_pos(lstm_out_pos.view(len(word_char_cat), -1))
        tag_space_ner = self.hidden2tag_ner(lstm_out_ner.view(len(word_char_cat), -1))
