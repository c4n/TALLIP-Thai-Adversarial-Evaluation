...
class LSTMTagger(nn.Module):
    ...
    def _lstm_compose_char_viz(self, char_seq):
        ...
        char_embeds.view(len(char_seq), 1, -1), hidden)
        attn_weight=self.attn_char(None,lstm_out)
        attn_e=self.attn_char.attn_energy(None,lstm_out)
        context = attn_weight.bmm(lstm_out.transpose(0, 1))
        return  attn_weight,attn_e,context

    def attn_viz(self, text_word_seq, text_char_seq):
        print("cubt")
        ...
        for word_char_seq in text_char_seq:
            attn_weight,attn_e,context = self._lstm_compose_char_viz(word_char_seq)
            ...
            char_tensor_list.append(attn_weight)
            e_list.append(attn_e)
            c_list.append(context)
        #char_tensor_list = torch.cat(tuple(char_tensor_list))
        return char_tensor_list,e_list,c_list
