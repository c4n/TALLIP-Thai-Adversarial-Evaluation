...
class ORCHIDTagger(nn.Module):
    ...
    def __init__(self, word_embedding_dim, hidden_dim, word_size, tagset_size_pos, tagset_size_type):
        ...
        self.lstm_pos_type = nn.LSTM(word_embedding_dim, hidden_dim // 2, bidirectional = True)
    ...
    def _lstm_pos_feats(self,word_char_cat):
        ...
        lstm_out, self.hidden_pos = self.lstm_pos_type(word_char_cat.view(len(word_char_cat), 1, -1), self.hidden_pos)

        tag_space_pos = self.hidden2tag_pos(lstm_out.view(len(word_char_cat), -1))
        tag_space_type = self.hidden2tag_type(lstm_out.view(len(word_char_cat), -1))
    ...
    def forward(self, text_word_seq):
        ...
        lstm_pos_feats,lstm_type_feats = self._lstm_pos_feats(word_emb)
