...
class LSTMTagger(nn.Module):
    ...
    def __init__(self,char_embedding_dim, word_embedding_dim, hidden_dim, char_size, word_size, tagset_size_pos, tagset_size_ner):
        ...
        options_file = '/home/can/Documents/thai_nlp_research/BASIL/options.json'
        weight_file = '/home/can/Documents/thai_nlp_research/BASIL/out_weights.hdf5'
        ...
        self.lstm_pos_ner = nn.LSTM(128, hidden_dim // 2, bidirectional = True)
    ...
    def _lstm_pos_ner_feats(self,word_char_cat):
        ...
        #print(word_char_cat.squeeze().shape,word_char_cat.squeeze().view(len(word_char_cat.squeeze()), 1, -1).shape)
#         lstm_out, self.hidden_pos_ner = self.lstm_pos_ner(word_char_cat.view(len(word_char_cat), 1, -1), self.hidden_pos_ner)
        
        lstm_out, self.hidden_pos_ner = self.lstm_pos_ner(word_char_cat.squeeze().view(len(word_char_cat.squeeze()), 1, -1), self.hidden_pos_ner)
        tag_space_pos = self.hidden2tag_pos(lstm_out.view(len(word_char_cat.squeeze()), -1))
        tag_space_ner = self.hidden2tag_ner(lstm_out.view(len(word_char_cat.squeeze()), -1))
    ...
    def forward(self, text_word_seq):
        ...
        lstm_pos_feats, lstm_ner_feats = self._lstm_pos_ner_feats(word_emb['elmo_representations'][0])
