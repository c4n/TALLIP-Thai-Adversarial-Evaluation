...
from basil.models.multitasks.backoff_lstm_affix_rnn_init_no_decomp import LSTMTagger
...
class TrainModel:
    ...
    def __init__(self,model,corpus, model_weights=None):
        np.random.seed(42)
    ...
    def train(self, epoch_num:int):
        ...
        #         input_text = input_train_word
            ...
            input_syl = self.prep_torch.input_train_syl

            for text_file, syl_text_file, pos_tags, ner_tags in zip(input_text,input_syl, self.prep_torch.pos_train, self.prep_torch.ner_train):
                ...
                syl_in = [self.prep_torch.prepare_sequence_syl(word) for word in syl_text_file]
                ...
                tag_scores_pos,tag_scores_ner = self.model(word_in,syl_in,char_in,prefix_in)
        ...
        #             logger.scalar_summary(tag, value, count)
            ...
            with torch.no_grad():
                y_pred_pos = []
                y_pred_ner = []
                for text, syl_text in zip(self.prep_torch.input_dev_word,self.prep_torch.input_dev_syl):
            ...
            print("true",self.y_true_pos[:10])
            print("pred",y_pred_pos[:10])
            ...
            if f1_pos+f1_ner > best_f1:
                torch.save(self.model.state_dict(), "model_oct_2018best1_affix_syllable_attn.p")#save best model only
