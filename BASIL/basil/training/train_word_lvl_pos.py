    ...
    def train(self, epoch_num:int):
        ...
        #         input_text = input_train_word
            ...
            for text_file, pos_tags, ner_tags in zip(input_text, self.prep_torch.pos_train, self.prep_torch.ner_train):
                ...
                tag_scores_pos = self.model(word_in,char_in)
        ...
        #             logger.scalar_summary(tag, value, count)
            ...
            with torch.no_grad():
                ...
                for text in self.prep_torch.input_dev_word:
                    ...
                    pred_pos = self.model(wtest_in,ctest_in)
            ...
            print("F1 (POS):",f1_pos)
            if f1_pos > best_f1:
                torch.save(self.model.state_dict(), "model_oct_word_2018best1_wordlvl_JUSTPOS.p")#save best model only
                best_f1 = f1_pos
