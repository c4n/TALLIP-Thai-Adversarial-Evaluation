...
class TrainModel:
    ...
    def __init__(self,model,corpus,seed_num):
        ...
        self.model = model(WORD_EMBEDDING_DIM,HIDDEN_DIM,\
                          len(self.prep_torch.word_to_index),\
                           len(self.prep_torch.pos_to_index),  len(self.prep_torch.type_to_index)).to(device)
        self.loss_function = nn.NLLLoss()
        learning_rate = 0.001
        self.optimizer = cprop.CPropAdam(self.model.parameters(), lr=learning_rate, cprop_beta=0.999, cprop_c=1, cprop_cdf='normal')
...
#                 print("text",len(text_file))
                ...
                tag_scores_pos, tag_scores_type = self.model(word_in)
        ...
        #             logger.scalar_summary(tag, value, count)
            ...
            with torch.no_grad():
                ...
                for text in self.prep_torch.input_dev_word:
                    ...
                    pred_pos, pred_type = self.model(wtest_in)
            ...
            print("F1 (POS):",f1_pos)
            if f1_pos > best_f1:
                torch.save(self.model.state_dict(), "weights/model_bilstm_baseline_cprop_orchid_seed"+str(self.seed_num)+".p")#save best model only
                best_f1 = f1_pos
