...
#                 prefix_in = self.prep_torch.prepare_sequence_feat_prefix(text_file)
                ...
                tag_scores_pos, tag_scores_type = self.model(word_in,char_in)
...
#                     ptest_in = self.prep_torch.prepare_sequence_feat_prefix(text)
                    pred_pos, pred_type = self.model(wtest_in,ctest_in)
            ...
            print("F1 (POS):",f1_pos)
            if f1_pos > best_f1:
                torch.save(self.model.state_dict(), "weights/model_BILSTM_U_BA_clp_orchid_seed"+str(self.seed_num)+".p")#save best model only
                best_f1 = f1_pos
